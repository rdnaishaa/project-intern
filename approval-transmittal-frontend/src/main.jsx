...
const initialDocs = [
  {
    ...
    steps: [
      { id: 11, order: 1, approverId: 2, approverName: "Budi", position: "HRBP Manager", area: "Bontang", status: "APPROVED", comment: "Disetujui.", signedAt: "2026-08-24T08:30:00", signature: "Budi Signature" },
...
function Detail({user,doc,onBack,onApprove,verify}) {
  const [signature,setSignature]=useState(""),[comment,setComment]=useState("");
  if(!doc)return <div>Dokumen tidak ditemukan.</div>;
  const step=doc.steps.find(s=>s.approverId===user.id&&s.status==="WAITING");
  const active=step && doc.steps.filter(s=>s.order<step.order).every(s=>s.status==="APPROVED");
  return <><button onClick={onBack} className="mb-4 text-sm font-semibold text-[#1261A0]">← Kembali</button><div className="mb-5 flex flex-col justify-between gap-3 md:flex-row md:items-end"><div><div className="text-xs font-bold uppercase text-slate-400">{doc.submissionNo}</div><h1 className="mt-1 text-2xl font-extrabold">{doc.title}</h1><p className="text-sm text-slate-500">{doc.type} · {doc.department} · {doc.area}</p></div><Badge status={doc.status}/></div>
    <div className="grid gap-5 xl:grid-cols-[1.25fr_.75fr]">
      <div className="space-y-5"><div className="card p-5"><h2 className="font-bold">Informasi Dokumen</h2><div className="mt-4 grid gap-4 sm:grid-cols-2 text-sm"><div><small className="text-slate-400">Pemohon</small><div className="font-semibold">{doc.applicantName}</div></div><div><small className="text-slate-400">Tanggal</small><div className="font-semibold">{fmt(doc.createdAt)}</div></div><div className="sm:col-span-2"><small className="text-slate-400">Keperluan</small><div>{doc.description||"-"}</div></div></div><div className="mt-4 rounded-xl bg-slate-50 p-3 text-sm"><FileText className="inline mr-2" size={16}/>{doc.fileName}</div></div>
      <div className="card p-5"><h2 className="font-bold">Approval Chain</h2><div className="mt-5 space-y-4">{doc.steps.map((s,i)=><div key={s.id} className="relative flex gap-4">{i<doc.steps.length-1&&<div className="absolute left-4 top-9 h-full w-px bg-slate-200"/>}<div className={`z-10 grid h-8 w-8 shrink-0 place-items-center rounded-full ${s.status==="APPROVED"?"bg-emerald-100 text-emerald-700":s.status==="REJECTED"?"bg-red-100 text-red-700":"bg-slate-100 text-slate-500"}`}>{s.status==="APPROVED"?<Check size={16}/>:s.order}</div><div className="flex-1 rounded-xl border p-3"><div className="flex justify-between gap-2"><div><b>{s.approverName}</b><div className="text-xs text-slate-500">{s.position} · {s.area}</div></div><Badge status={s.status}/></div>{s.signedAt&&<div className="mt-2 text-xs text-slate-400">{fmt(s.signedAt)}</div>}{s.comment&&<div className="mt-2 rounded-lg bg-slate-50 p-2 text-sm"><b>Catatan:</b> {s.comment}</div>}{s.signature&&<div className="mt-2 text-xs text-emerald-700">✓ Signature evidence recorded</div>}</div></div>)}</div></div></div>
      <div>{active&&doc.status==="IN_APPROVAL"&&<div className="card border-blue-200 p-5"><h2 className="font-bold text-[#1261A0]">Tindakan Approval — Step {step.order}</h2><p className="mt-1 text-sm text-slate-500">Approve memerlukan signature evidence.</p><div className="mt-4"><label className="label">Tanda Tangan</label><SignaturePad onChange={setSignature}/></div><div className="mt-4"><label className="label">Catatan</label><textarea className="input min-h-24" value={comment} onChange={e=>setComment(e.target.value)} placeholder="Wajib jika Reject"/></div><div className="mt-4 grid grid-cols-2 gap-2"><button className="btn-danger" onClick={()=>{if(!comment.trim())return alert("Catatan wajib untuk Reject.");onApprove(doc.id,step.id,"REJECT",null,comment)}}>Reject</button><button className="btn-primary" onClick={()=>{if(!signature)return alert("Tanda tangan wajib.");onApprove(doc.id,step.id,"APPROVE",signature,comment)}}>Approve <Check size={16}/></button></div></div>}
      {doc.status==="APPROVED"&&<div className="card border-emerald-200 bg-emerald-50/50 p-5"><div className="flex items-center gap-2 font-bold text-emerald-700"><CheckCircle2/> Semua approval selesai</div><p className="mt-2 text-sm text-slate-600">QR digunakan untuk membuka halaman verification, bukan dimasukkan ke surat.</p><div className="mt-4 rounded-xl bg-white p-4 text-center"><QRCodeSVG value={doc.qr} size={190}/><div className="mt-3"><button className="btn-secondary" onClick={verify}><QrCode size={16}/>Buka Verification</button></div></div></div>}</div>
    </div></>;
}
